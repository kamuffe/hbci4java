		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RKUmsDelimiters.class );
	
	private String rewriteKUms( String st )
	{
		logger.debug( "rewriting statement of account" );
		...
		if ( !temp.substring( temp.length() - 3, temp.length() ).equals( "\r\n-" ) )
		{
			...
			if ( posi >= 0 )
			{
				...
				{
					logger.debug( "absolutely no ending sequence found - " +
							"maybe statement of account splitted at wrong position?" );
				}
			}
			else
			{
				logger.warn( "statement of account seems to be empty" );
			}
		}
		...
		{
			logger.debug( "this institute produces buggy account statements!");
			logger.debug( "wrongCRLF:" + wrongCRLF
