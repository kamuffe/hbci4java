		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RMissingMsgRef.class );
	...
	public String incomingCrypted( String st, MsgGen gen )
	{
		...
		if ( idx != -1 )
		{
			try
			{
				...
				if ( plusidx == -1 )
				{
					logger.warn( "MsgRef is missing, adding it" );
					...
					for ( int i = 0; i < 2; i++ )
					{
						...
						logger.warn( "setting MsgRef." + des[i] + " to " + temp );
						msghead_st += (i == 0 ? "+" : ":");
						msghead_st += temp;
					}
