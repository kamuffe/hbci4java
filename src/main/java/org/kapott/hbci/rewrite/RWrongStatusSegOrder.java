		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RWrongStatusSegOrder.class );
	...
	{
		List<Properties> segmentList = new ArrayList<>();
	...
	public String incomingClearText( String st, MsgGen gen )
	{
		...
		List<Properties> headerList = new ArrayList<>();
		List<Properties> HIRMGList = new ArrayList<>();
		List<Properties> HIRMSList = new ArrayList<>();
		List<Properties> dataList = new ArrayList<>();
		...
		for ( Iterator<Properties> i = segmentList.iterator(); i.hasNext(); )
		{
			...
			if ( segmentCode.equals( "HNHBK" ) || segmentCode.equals( "HNSHK" ) )
			{
				...
				if ( !inHeader )
				{
					logger.warn( "RWrongStatusSegOrder: found segment {} at invalid position", segmentCode );
					errorOccured = true;
				}
		...
		if ( errorOccured )
		{
			...
			newMsg.append( getDataForSegmentList( st, headerList, counter ) );
			counter += headerList.size();

			// HIRMG-segment
			newMsg.append( getDataForSegmentList( st, HIRMGList, counter ) );
			counter += HIRMGList.size();

			// HIRMS-segmente
			newMsg.append( getDataForSegmentList( st, HIRMSList, counter ) );
			counter += HIRMSList.size();

			// restliche daten-segmente
			newMsg.append( getDataForSegmentList( st, dataList, counter ) );

			logger.debug( "RWrongStatusSegOrder: new message after reordering: {}", newMsg );
		}
		else
		{
	...
	private String getDataForSegmentList( String origMsg, List<Properties> list, int counter )
	{
		...
		for ( Iterator<Properties> i = list.iterator(); i.hasNext(); )
		{
			...
			StringBuilder segmentData = new StringBuilder( origMsg.substring( start, start + len ) );
