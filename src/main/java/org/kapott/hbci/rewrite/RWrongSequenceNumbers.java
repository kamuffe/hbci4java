		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RWrongSequenceNumbers.class );
		...
		for ( int i = 0; i < sb.length(); i++ )
		{
			...
			else if ( !quoteNext && ch == '\'' || i == 0 )
			{
				...
				if ( idx != -1 )
				{
					...
					if ( seq != correctSeq )
					{
						logger.warn( "found wrong sequence number " + seq + "; replacing with " + correctSeq );
