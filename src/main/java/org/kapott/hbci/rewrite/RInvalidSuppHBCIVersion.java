		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RInvalidSuppHBCIVersion.class );
	...
	public String incomingClearText( String st, MsgGen gen )
	{
		...
		{
			logger.info( "unable to apply rewriter " + this.getClass().getSimpleName() + " - leaving messag unchanged" );
			logger.debug( "", e );
		}
		finally
		{
	 ...
	 *        die Nachricht.
	 * @param elem
	 *        Das Element mit der HBCI-Version.
	 * @param search
	 *        die zu suchende HBCI-Version.
	 * @param replace
	 *        die HBCI-Version, die stattdessen eingetragen werden soll.
	...
	private boolean replace( StringBuffer msg, SyntaxElement elem, String search, HBCIVersion replace )
	{
		...
		logger.warn( "there is an invalid hbci version number ('" + version + "') in this BPD - replacing it with '" + s + " '" );

		int startpos = elem.getPosInMsg() + 1; // +1 wegen überspringen des pre-delimiters
		msg.replace( startpos, startpos + 1, s );
		logger.debug( "new message after replacing: {}", msg );
		return true;
	}
}
