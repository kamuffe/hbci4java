		...
		extends Rewrite
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RSecTypeTAN.class );
	...
	public String incomingClearText( String st, MsgGen gen )
	{
		...
		try
		{
			...
			for ( int i = 0;; i++ )
			{
				...
				{ // "TAN" ist ungültiger Bezeichner
					logger.warn( "there is an invalid sec type (TAN) in this BPD - removing it" );
					...
					if ( number > 1 )
					{ // nur das eine fehlerhafte TAN:1 löschen
					...
					{ // komplettes segment "SecMethod" löschen
					...
					logger.debug( "new message after removing: {}", st );
					break;
				}
			}
