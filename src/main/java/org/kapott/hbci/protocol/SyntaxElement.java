 ...
 * selbst, eine segmentfolge, ein einzelnes Segment, eine Datenelementgruppe (DEG) oder
 * ein einzelnes Datenelement (DE))
 */
public abstract class SyntaxElement
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( SyntaxElement.class );
	 ...
	 * auftauchen muessten
	 */
	protected abstract MultipleSyntaxElements parseNewChildContainer( Node ref, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
	...
	private void initData( String type, String name, String ppath, int idx, Document syntax )
	{
		...
		{
			logger.debug( "creating segment " + ppath + " -> " + name + "(" + idx + ")" );			
		}
				...
				for ( Node ref = def.getFirstChild(); ref != null; ref = ref.getNextSibling() )
				{
					if ( ref.getNodeType() == Node.ELEMENT_NODE )
					{
						...
						if ( child != null )
						{
							...
							{
								logger.debug( "child container " + child.getPath() + " has syntaxIdx=" + child.getSyntaxIdx() );								
							}
						}
						syntaxIdx++;
					}
	...
	private void initData( String type, String name, String ppath, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
	 ...
	 */
	protected SyntaxElement( String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
			Hashtable<String, String> valids )
	{
		initData( type, name, path, predelim, idx, res, fullResLen, syntax, predefs, valids );
	}

	protected void init( String type, String name, String path, char predelim, int idx, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
			Hashtable<String, String> valids )
	{
		initData( type, name, path, predelim, idx, res, fullResLen, syntax, predefs, valids );
	}

	protected MultipleSyntaxElements parseAndAppendNewChildContainer( Node ref, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax,
			Hashtable<String, String> predefs, Hashtable<String, String> valids )
	{
		...
		else
		{
			...
			if ( destPath.startsWith( getPath() ) )
			{
				...
				if ( !ret && tryToCreate )
				{
					...
					logger.debug( "  subname is " + subType );
					...
					if ( !found )
					{
						...
						if ( found )
						{
							...
							logger.debug( "  inserting child container with syntaxIdx " + newChildIdx + " at position " + newPosi );
					...
					{
						logger.debug( "  subtype " + subType + " already existing - will not try to create" );
					}
				}
			}
		}
