...
public abstract class MultipleSyntaxElements
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( MultipleSyntaxElements.class );
	...
	private void initData( Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
				...
				catch ( ParseErrorException e )
				{
					...
					if ( save.length() > 1 )
					{
						...
						if ( (this instanceof MultipleDEGs) && this.minnum == 0 && this.maxnum > 1 && idx > 1 )
						{
							...
							{
								logger.debug( "applying shortcut for optional MultipleDEGs, have no more content in according range" );
	 ...
	 */
	protected MultipleSyntaxElements( Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
			Hashtable<String, String> valids )
	{
		initData( ref, path, predelim0, predelim1, res, fullResLen, syntax, predefs, valids );
	}

	protected void init( Node ref, String path, char predelim0, char predelim1, StringBuffer res, int fullResLen, Document syntax, Hashtable<String, String> predefs,
			Hashtable<String, String> valids )
	{
		initData( ref, path, predelim0, predelim1, res, fullResLen, syntax, predefs, valids );
	}
