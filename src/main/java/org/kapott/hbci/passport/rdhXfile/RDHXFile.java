...
public class RDHXFile
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( RDHXFile.class );
	...
	public RDHXFile( byte[] data, byte[] passphrase )
	{
		...
		while ( posi < len )
		{
			...
			if ( tag == 0x4e56L )
			{
				...
				logger.debug( "found diskhead field" );
			...
			else if ( tag == 0x564bL )
			{
				...
				logger.debug( "found hbciaccount field" );
			...
			else if ( tag == 0x53d6L )
			{
				...
				logger.debug( "found bankkeys field" );
		...
		if ( macfield != null )
		{
			...
			logger.debug( "MAC field ok: {}", macOK );
			if ( !macOK )
			{
		...
		{
			logger.error( "RDH-2/10-file does not contain a MAC field - ignoring this for now" );
		}
	...
	private byte[] deriveKey( int dkLen, String algname )
	{
		...
		logger.debug( "calculating key with alg {} and length {}", algname, dkLen );
	...
	private byte[] calculateMAC()
	{
		try
		{
			...
			logger.debug( "using {}-byte-key for MAC calculation", keysize );

			// TODO: HmacSHA256 is available only in Java-6++
			String algname = (pversion == 2) ? "HmacSHA1" : "HmacSHA256";
			logger.debug( "MAC algorithm is {}", algname );
