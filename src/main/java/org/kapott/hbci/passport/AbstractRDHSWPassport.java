		...
		extends AbstractRDHPassport
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( AbstractRDHSWPassport.class );
	
	protected HBCIKey[][] keys;

	protected AbstractRDHSWPassport( HBCICallback callback, Object init )
	{
		super( callback, init );
	...
	{
		// System.out.println("passportRDH: setting key "+i+","+j+" to
	...
	public byte[] decrypt( byte[] cryptedKey, byte[] cryptedMsg )
	{
		try
		{
			...
			{
				logger.debug( "decrypting message key with (n,d)-algorithm" );
			...
			{
				logger.debug( "decrypting message key with (p,q,dP,dQ,qInv)-algorithm" );
	...
	private int getKeySizeByProfile()
	{
		...
		logger.debug( "using keysize " + ret + " bits for newly generated keys" );
		return ret;
	}
	...
	public HBCIKey[][] generateNewUserKeys()
	{
		...
		try
		{
			logger.info( "generating new user keys" );
