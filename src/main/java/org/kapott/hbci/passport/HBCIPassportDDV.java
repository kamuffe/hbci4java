...
/**
 * <p>
 * Passport-Klasse für Sicherheitsverfahren DDV mit Medium Chipkarte. Bei dieser
 ...
 * und für die Verschlüsselung der Nachrichten.
 * </p>
 * <p>
 ...
 * diese Zugangsdaten auf einer einzigen Chipkarte gespeichert werden können.
 * </p>
 * <p>
 ...
 * Prefix (Pfad) und der Seriennummer der Chipkarte zusammen.
 * </p>
 */
		...
		extends AbstractDDVPassport
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIPassportDDV.class );
	...
	public HBCIPassportDDV( HBCICallback callback, Object init )
	{
		...
		logger.debug( "trying to load native DDV library {}", ddvLib);
		System.load( ddvLib );

		// init card
		logger.debug( "using chipcard terminal with port " + comport + " and terminal number " + ctnumber );
		try
		{
			getCallback().callback( this,
					HBCICallback.NEED_CHIPCARD,
					HBCIUtils.getLocMsg( "CALLB_NEED_CHIPCARD" ),
		...
		{
			logger.warn( "error while reading passport file" );
		}
	...
	public void setFileName( String filename )
	{
		...
		if ( filename != null && filename.length() > 0 )
		{
			...
			{
				logger.info( "auto-fixed passport filename from " + filename + " to " + newName );
				filename = newName;
			}
		}
	...
	public byte[][] encrypt( byte[] plainMsg )
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_CANTCRYPT" ), ex );
		}
	}
	...
	public byte[] decrypt( byte[] cryptedKey, byte[] cryptedMsg )
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_DECRYPTERR" ), ex );
		}
	}
