		...
		extends HBCIPassportDDV
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIPassportDDVPCSC.class );
		...
		try
		{
			...
			if ( file.exists() && file.isFile() && file.canRead() )
			{
				...
				while ( true ) // loop for entering the correct passphrase
				{
					...
					{
						throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PASSPORT_READERR" ), e2 );
					}
