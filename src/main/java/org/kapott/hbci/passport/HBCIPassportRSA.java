		...
		implements HBCIPassportChipcard
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIPassportRSA.class );

	protected static final byte[] CIPHER_SALT = { (byte) 0x56, (byte) 0xbc, (byte) 0x1c, (byte) 0x88,
			(byte) 0x1f, (byte) 0xe3, (byte) 0x73, (byte) 0xcc };
	protected static final int CIPHER_ITERATIONS = 987;

	public static final int KEY_INST_SIG = 0;
	public static final int KEY_INST_ENC = 1;
	public static final int KEY_MY_PUBLIC_SIG = 2;
	public static final int KEY_MY_PUBLIC_ENC = 3;
	 ...
	 * @param dummy
	 */
	public HBCIPassportRSA( HBCICallback callback, Object init, int dummy )
	{
		super( callback, init );
	 ...
	 * @param init
	 */
	public HBCIPassportRSA( HBCICallback callback, Object init )
	{
		...
		try
		{
			...
			////////////////////////////////////////////////////////////////////////
			// init card
			logger.debug( "initializing javax.smartcardio" );
			getCallback().callback( this, HBCICallback.NEED_CHIPCARD, HBCIUtils.getLocMsg( "CALLB_NEED_CHIPCARD" ), HBCICallback.TYPE_NONE, null );
			...
			{
				throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PASSPORT_INSTDATAERR" ), e );
			}
			//
			////////////////////////////////////////////////////////////////////////
			...
			logger.debug( "loading passport data from file {}", getFileName() );
			...
			{ // loop for entering the correct passphrase
				...
				{
					throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PASSPORT_READERR" ), e );
				}
		...
		catch ( Exception e )
		{
			...
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_CTERR" ), e );
		}
		finally
		{
			// Close Passport-File
	...
	public String getProfileVersion()
	{
		...
		if ( result == null )
		{
			logger.debug( "no RDH profile version explicity specified - starting autodetection" );
			...
			{ // TODO: support FinTS-4, too
				result = "1";
				setProfileVersion( result );
				logger.debug( "this is HBCI version '" + getHBCIVersion() + "', which only supports RDH-1" );
			}
			else
			{
				...
				if ( key != null )
				{
					...
					result = key.num;
					setProfileVersion( result );
					logger.debug( "using user sig key num '" + result + "' as profile version" );
				}
				else
				{
					key = getInstEncKey();
					if ( key != null && (key.num.equals( "1" ) || key.num.equals( "2" ) || key.num.equals( "10" )) )
					{
						...
						result = key.num;
						logger.debug( "using inst enc key num '" + result + "' as RDH profile version" );
					}
					else
					{
						// neither user keys nor inst keys present - using highest available profile
						logger.debug( "no keys found in passport - so we use the highest available profile" );
						...
						logger.debug( "using RDH profile '" + result + "' taken from supported profiles (BPD)" );
					}
				}
			}
		}
		...
		{
			logger.debug( "using forced RDH profile version '" + result + "'" );
		}
	...
	public void saveBankData()
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PASSPORT_INSTSAVEERR" ), e );
		}
	}
	...
	public void saveChanges()
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PASSPORT_WRITEERR" ), e );
		}
	}
	...
	private void checkPIN()
	{
			...
			if ( !isPINEntered() )
			{
				if ( getUseSoftPin() == 1 )
				{
					...
					if ( pin == null || pin.length() == 0 )
					{
						...
						getCallback().callback( this,
								...
								HBCIUtils.getLocMsg( "CALLB_NEED_SOFTPIN" ),
				...
				else
				{
					getCallback().callback( this,
							...
							HBCIUtils.getLocMsg( "CALLB_NEED_HARDPIN" ),
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_PINERR" ), e );
		}
	}
	...
	protected void ctReadKeyData()
	{
		...
		{
			logger.debug( "found valid instSigKey " );
		...
		{
			logger.debug( "found valid instEncKey " );
		...
		{
			logger.debug( "found valid myPublicSigKey " );
		...
		{
			logger.debug( "found valid myPublicEncKey " );
