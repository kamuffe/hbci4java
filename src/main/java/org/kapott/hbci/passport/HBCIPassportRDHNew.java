...
/**
 * <p>
 * Passport-Klasse für RDH-Zugänge mit Sicherheitsmedium "Datei". Bei dieser Variante
 ...
 * </p>
 * <p>
 * Die Schlüsseldateien, die <em>HBCI4Java</em> mit dieser Klasse erzeugt und verwaltet, sind
 ...
 * </p>
 * <p>
 * Ein weiterer Ausweg aus diesem Problem wäre, eine technische Beschreibung des
 ...
 * implementiert werden kann, die mit Schlüsseldateien dieser "anderen" Software arbeiten kann.
 * </p>
 * 
		...
		extends AbstractRDHSWFileBasedPassport
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIPassportRDHNew.class );

	private String profileVersion;

	public HBCIPassportRDHNew( HBCICallback callback, Object init, int dummy )
	{
		super( callback, init );
	}

	public HBCIPassportRDHNew( HBCICallback callback, Object initObject )
	{
		...
		logger.debug( "loading passport data from file {}", filename );
		setFilename( filename );

		if ( init )
		{
			logger.debug( "loading data from file {}", filename );
			...
			if ( n.getNodeType() == Node.ELEMENT_NODE )
			{
						...
						keynode.getAttribute( "part" ).equals( part ) )
				{
					...
					else
					{
						...
						{
							// only CRT
							logger.debug( "private " + type + " key is CRT-only" );
						...
						{
							// only exponent
							logger.debug( "private " + type + " key is exponent-only" );
						...
						{
							// complete data
							logger.debug( "private " + type + " key is fully specified" );
	...
	protected void createKeyElement( Document doc, Element root, String owner, String type, String part, HBCIKey key )
	{
		if ( key != null )
		{
			...
			else
			{
				...
				{
					logger.debug( "saving " + type + " key as fully specified" );
				...
				{
					logger.debug( "saving " + type + " key as exponent-only" );
				...
				{
					logger.debug( "saving " + type + " key as crt-only" );
				...
				{
					logger.warn( "key has none of the known types - please contact the author!" );
				}
			}
		}
	}
	...
	public String getProfileVersion()
	{
		...
		if ( ret.length() == 0 )
		{
			logger.debug( "have to determine my rdh-profile-version, but have no information about it yet" );
			...
			if ( hasMySigKey() )
			{
				logger.debug( "found user sig key in passport file, but no profile version, " +
			...
			{
				logger.debug( "no user keys found in passport - so we use the highest available profile" );
				...
				logger.debug( "using RDH profile " + ret + " taken from supported profiles (BPD)" );
			}
		}
		...
		if ( ret == null || ret.length() == 0 )
		{
			...
			logger.warn( "unable to determine rdh-profile-version using BPD, using default version {}", ret );
		}
