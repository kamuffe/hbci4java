...
/**
 * <p>
 * Ergebnisse der Abfrage von Kontoumsatzinformationen.
 ...
 * </p>
 * <p>
 * Es können auch alle Umsatzzeilen in einer einzigen Liste abgefragt werden (also nicht
 * in Buchungstage unterteilt .
 * </p>
 */
		...
		extends HBCIJobResultImpl
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( GVRKUms.class );
			...
			implements Serializable
	{
		 ...
		 * Das ist genau dann der Fall, wenn der Wert von <code>gvcode</code> gleich <code>999</code> ist.
		 * </p>
		 * <p>
		 * Wenn die Zusatzinformationen aber ausgewertet werden können (und <code>gvcode!=999</code>),
			...
			implements Serializable
	{
		 ...
		 * komplett in das <code>number</code>-Feld geschrieben.
		 * </p>
		 */
		public Konto my;
	...
	private void verifyMT94xParsing( String where )
	{
		...
		if ( restMT940 != null && restMT940.length() != 0 )
		{
			...
			logger.debug( "restMT940: " + restMT940 );
		}
		...
		if ( restMT942 != null && restMT942.length() != 0 )
		{
			logger.warn(
					where +
							...
							"to see the data that could not be parsed." );
			logger.debug( "restMT942: " + restMT942 );
		}
	}
	...
	private void parseMT94x( StringBuffer buffer, List<BTag> tage, StringBuffer rest )
	{
		...
		logger.debug( "now parsing MT94x data" );
		...
		try
		{
			...
			while ( buffer.length() != 0 )
			{
				...
				// extract konto data
				String kontoInfo = Swift.getTagValue( st_tag, "25", 0 );
				int pos = kontoInfo.indexOf( '/' );
				String blz;
				String number;
				String iban;
				String curr;
				...
				if ( pos != -1 )
				{
					blz = kontoInfo.substring( 0, pos );
					number = kontoInfo.substring( pos + 1 );
					iban = "";
					curr = "";
				...
				while ( true )
				{
					...
					else
					{
						...
						// FE: ein Monat reicht nicht, es sollte schon ein halbes Jahr sein - es gab verschiedene Probleme mit Umsaetzen im
						// falschen Jahr!!
					...
					if ( st_multi != null )
					{
						...
						if ( !line.gvcode.equals( "999" ) )
						{
							...
							if ( acc.blz != null )
							{
								...
								if ( space != -1 )
								{
									logger.debug( "blz/bic \"" + acc.blz + "\" contains invalid chars, trimming after first space" );
		...
		{
			logger.error( "There is unparsed MT94x data - an exception occured while parsing" );
			logger.debug( "current MT94x buffer: " + buffer );
