...
public final class HBCIDialog
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIDialog.class );
	...
	{
		logger.debug( "creating new dialog" );
	...
	private HBCIMsgStatus doDialogInit()
	{
		...
		try
		{
			...
			if ( ret.isOK() )
			{
				...
				logger.debug( "dialog-id set to " + dialogid );
				...
				for ( int i = 0; true; i++ )
				{
					...
					parentHandler.getPassport().getCallback().callback( mainPassport,
			...
			parentHandler.getPassport().getCallback().status( mainPassport, HBCICallback.STATUS_DIALOG_INIT_DONE, new Object[] { ret, dialogid } );
		}
		catch ( Exception e )
		{
			ret.addException( e );
	...
	{
		logger.info( HBCIUtils.getLocMsg( "LOG_PROCESSING_JOBS" ) );
		...
		for ( int j = 0; j < nof_messages; j++ )
		{
			...
			while ( true )
			{
				...
				try
				{
					logger.debug( "generating custom msg #" + (j + 1) + " (loop " + (loop + 1) + ")" );
					...
					for ( Iterator<HBCIJobImpl> i = tasks.iterator(); i.hasNext(); )
					{
						...
						if ( task.needsContinue( loop ) )
						{
							...
							parentHandler.getPassport().getCallback().status( mainPassport, HBCICallback.STATUS_SEND_TASK, task );
							task.setIdx( taskNum );
					...
					{
						logger.error( "aborting current loop because of errors" );
						break;
					}
	...
	private HBCIMsgStatus doDialogEnd()
	{
		...
		try
		{
			logger.info( HBCIUtils.getLocMsg( "LOG_DIALOG_END" ) );
			parentHandler.getPassport().getCallback().status( mainPassport, HBCICallback.STATUS_DIALOG_END, null );
	...
	private int getTotalNumberOfGVSegsInCurrentMessage()
	{
		...
		logger.debug( "there are currently " + total + " GV segs in this message" );
		return total;
	}
	...
	public void addTask( HBCIJobImpl job )
	{
		...
		try
		{
			logger.info( HBCIUtils.getLocMsg( "EXCMSG_ADDJOB", job.getName() ) );
		...
		catch ( Exception e )
		{
			...
			logger.error( "task " + job.getName() + " will not be executed in current dialog" );
		}
	}
	...
	public void newMsg()
	{
		logger.debug( "starting new message" );
		msgs.add( new ArrayList<HBCIJobImpl>() );
		listOfGVs.clear();
	}
