...
/**
 * <p>
 * Ein Handle für genau einen HBCI-Zugang. Diese Klasse stellt das Verbindungsglied
 ...
 * Darin sind folgende Daten zusammengefasst:
 * <ul>
 ...
 * </ul>
 * <p>
 * Alle Anfragen der Anwendung an den HBCI-Kernel laufen über einen solchen
		...
		implements IHandlerData, AutoCloseable
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIHandler.class );
	...
	private Map<String, HBCIDialog> dialogs = new HashMap<>();
	...
	public HBCIHandler( HBCIPassport passport )
	{
		...
		try
		{
			...
			this.kernel = new HBCIKernelImpl( this );
	...
	public void updateMetaInfo()
	{
		...
		if ( bpd == null )
		{
			logger.warn( "have no bpd, skip fetching of meta info" );
			return;
		}
		...
		try
		{
			...
			{
				logger.info( "fetching SEPA information" );
				HBCIJob sepainfo = new GVSEPAInfo( this );
				sepainfo.addToQueue();
			}
			...
			{
				logger.info( "fetching TAN media list" );
				HBCIJob tanMedia = new GVTANMediaList( this );
				tanMedia.addToQueue();
			}
			...
			{
				logger.error( "error while fetching meta info: {}", status );
			}
		}
		catch ( Exception e )
		{
	...
	private void registerInstitute()
	{
		try
		{
			logger.debug( "registering institute" );
	...
	/**
	 * <p>
	 * Schließen des Handlers. Diese Methode sollte immer dann aufgerufen werden,
	 * wenn die entsprechende HBCI-Verbindung nicht mehr benötigt wird.
	 * </p>
	 * <p>
	...
	private String fixUnspecifiedCustomerId( String customerId )
	{
		if ( customerId == null )
		{
			...
			logger.debug( "using default customerid {}", customerId );
		}
		return customerId;
	}
	...
	private HBCIDialog getDialogFor( String customerId )
	{
		...
		if ( dialog == null )
		{
			logger.debug( "have to create new dialog for customerid {}", customerId );
	 ...
	 * Normalerweise muss diese Methode niemals manuell aufgerufen zu werden!
	 * </p>
	 * <p>
	...
	{
		logger.debug( "have to create new message for dialog for customer {}", customerId );
	 ...
	 * </p>
	 * <p>
	 * Eine Beschreibung aller unterstützten Geschäftsvorfälle befindet sich
	 * im Package <code>org.kapott.hbci.GV</code>.
	 * </p>
	 * 
	 * @param jobname
	...
	{
		logger.debug( "creating new job {}", jobname );
	...
	{
		logger.debug( "generating new lowlevel-job {}", gvname );
	...
	public void addJobToDialog( String customerId, HBCIJob job )
	{
		...
		finally
		{
			...
			if ( dialog != null )
			{
				...
				{
					logger.debug( "removing empty dialog for customerid " + customerId + " from list of dialogs" );
	 ...
	 * <p>
	 * Im Normalfall werden HBCI-Dialoge
	 ...
	 * und Dialog-Ende besteht.
	 * </p>
	 * <p>
	 * Ist die angegebene <code>customerId=null</code>, so wird der Dialog
	 * für die aktuell im Passport gespeicherte Customer-ID erzeugt.
	 * </p>
	 * 
	 * @param customerId
	...
	{
		customerId = fixUnspecifiedCustomerId( customerId );
		logger.debug( "creating empty dialog for customerid " + customerId );
	 ...
	 * </p>
	 * <p>
	 * Beim Hinzufügen der Aufträge zur Auftragsqueue (mit {@link org.kapott.hbci.GV.HBCIJob#addToQueue()}
	 ...
	 * </p>
	 * <p>
	 * Dazu wird eine Reihe von HBCI-Nachrichten mit dem HBCI-Server der Bank ausgetauscht. Die
	 ...
	 * </p>
	 * <p>
	 * Nach dem Nachrichtenaustausch wird ein Status-Objekt zurückgegeben,
	 * welches zur Auswertung aller ausgeführten Dialoge benutzt werden kann.
	 * </p>
	 * 
	...
	public HBCIExecStatus execute()
	{
		...
		try
		{
			...
			while ( !dialogs.isEmpty() )
			{
				...
				logger.debug( "executing dialog for customerid " + customerid );
		...
		finally
		{
			...
			{
				logger.error( "nested exception while closing passport: ", e );
			}
		}
	}
	...
	/**
	 * <p>
	 * Entspricht {@link #execute()}, allerdings können Callbacks hier auch synchron
	 ...
	 * synchron behandelt werden, terminiert diese Methode.
	 * </p>
	 * <p>
	 * Das zurückgegebene Status-Objekt zeigt an, ob diese Methode terminierte,
	...
	{
		logger.debug( "main thread: starting new threaded execute" );
		...
		new Thread()
		{
			public void run()
			{
				try
				{
					logger.debug( "hbci thread: starting execute()" );
				...
				finally
				{
					...
					logger.debug( "hbci thread: awaking main thread with hbci result data" );
					sync_main.setData( "callbackData", null );
					sync_main.stopWaiting();

					logger.debug( "hbci thread: thread finished" );
				}
			}
		}.start();
		...
		logger.debug( "main thread: waiting for hbci result or callback data from hbci thread" );
	...
	/**
	 * <p>
	 * Setzt bei Verwendung des threaded-callback-Mechanismus einen noch
	 ...
	 * </p>
	 * <p>
	 * Das führt dazu, dass der HBCI-Kernel die übergebenen Callback-Daten
	 ...
	 * </p>
	 * <p>
	 * Der Rückgabewert von <code>continueThreaded(String)</code> ist wieder
	 ...
	 * und zwar so lange, bis der HBCI-Dialog tatsächlich beendet ist.
	 * </p>
	 * <p>
	 * Mehr Informationen zu threaded callbacks in der Datei
	 * <code>README.ThreadedCallbacks</code>.
	 */
	public HBCIExecThreadedStatus continueThreaded( String retData )
	{
		logger.debug( "main thread: continuing hbci dialog with callback retData" );
		...
		logger.debug( "main thread: awaking hbci thread with callback data from application" );
		...
		logger.debug( "main thread: waiting for hbci result or new callback data from hbci thread" );
		sync_main.startWaiting( Integer.parseInt( HBCIUtils.getParam( "kernel.threaded.maxwaittime", "300" ) ), "no response from hbci thread - timeout" );
	 ...
	 * </ol>
	 * <p>
	 * Ist mindestens eine der beiden Bedingungen nicht erfüllt, so wird diese Methode mit einer
	 ...
	 * oder {@link #newKeys()} aufgerufen werden.
	 * </p>
	 * <p>
	 * Ab Version 2.4.0 von <em>HBCI4Java</em> muss der HBCIHandler nach dem
	 ...
	 * </p>
	 * <p>
	 * In jedem Fall muss für die neuen Schlüssel, die nach einer Schlüsselsperrung
	 * erzeugt werden, ein neuer INI-Brief generiert und an die Bank versandt werden.
	 * </p>
	 */
	public void lockKeys()
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_LOCKFAILED" ), ex );
		}
	}
	 ...
	 * </p>
	 * <p>
	 * <b>ACHTUNG!</b> Vor dieser Aktion sollte unbedingt ein Backup der aktuellen Schlüsseldatei
	 ...
	 * Schlüssel überschrieben.
	 * </p>
	 * <p>
	 * <b>ACHTUNG!</b> In noch ungünstigeren Fällen kann es auch vorkommen, dass neue Schlüssel
	...
	public void setKeys( KeyPair sigKey, KeyPair encKey )
	{
		...
		{
			throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_SETKEYS_ERR" ), ex );
		}
	}
	 ...
	 * 
	 * @param customerId
	 *        Kunden-ID, für die der Dialog ausgeführt werden soll
	...
	/**
	 * <p>
	 * Gibt die Namen aller vom aktuellen HBCI-Zugang (d.h. Passport)
	 * unterstützten Lowlevel-Jobs zurück. Alle hier zurückgegebenen Job-Namen
	 * können als Argument beim Aufruf der Methode
	 * {@link #newLowlevelJob(String)} benutzt werden.
	 * </p>
	 * <p>
	 * In dem zurückgegebenen Properties-Objekt enthält jeder Eintrag als
	 ...
	 * </p>
	 * <p>
	 * <em>(Prinzipiell unterstützt <em>HBCI4Java</em> für jeden
	 ...
	 * zu Debugging-Zwecken.)</em>
	 * </p>
	 * <p>
	 * Zum Unterschied zwischen High- und Lowlevel-Jobs siehe die
	 * Beschreibung im Package <code>org.kapott.hbci.GV</code>.
	 * </p>
	 * 
	...
	{
		Map<String, List<String>> allValidJobNames = kernel.getAllLowlevelJobs();
	 ...
	 * {@link #newLowlevelJob(String)} erzeugt.
	 * </p>
	 * <p>
	 ...
	 * eines Jobs via {@link #newLowlevelJob(String)} verwenden wird).
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * Die Methode <code>getLowlevelJobResultNames()</code> gibt nun alle gültigen Namen zurück,
	 ...
	 * </p>
	 * <p>
	 * Mit dem Tool {@link org.kapott.hbci.tools.ShowLowlevelGVRs} kann offline eine
	 ...
	 * ob das Ändern von terminierten Überweisungen erlaubt ist usw.).
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * I.d.R. werden mehrere Versionen eines Geschäftsvorfalles von der Bank
	 ...
	 * die <em>HBCI4Java</em> beim Erzeugen eines Jobs benutzt).
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param gvname
	 *        Lowlevel-Name des Geschäftsvorfalles, für den die Restriktionen
	 ...
	 * </p>
	 * <p>
	 * Eine Liste aller zur Zeit verfügbaren Highlevel-Jobnamen ist in der Paketbeschreibung
	 * des Packages <code>org.kapott.hbci.GV</code> zu finden. Wird hier nach einem Highlevel-Jobnamen
	 * gefragt, der nicht in dieser Liste enthalten ist, so wird eine Exception geworfen.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param selectX
	 *        kann aus einer Kombination (Addition) der Werte
