		...
		implements IHandlerData
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIInstitute.class );
	...
	{
		logger.debug( "extracting BPD from results" );
		...
		if ( p.size() != 0 )
		{
			...
			passport.setBPD( p );
			logger.info( "installed new BPD with version " + passport.getBPDVersion() );
	...
	void extractKeys( Properties result )
	{
		...
		try
		{
			logger.debug( "extracting public institute keys from results" );
	...
	private void doDialogEnd( String dialogid, boolean needSig )
	{
		...
		if ( !status.isOK() )
		{
			logger.error( "dialog end failed: " + status.getErrorString() );
	...
	private boolean isBPDExpired()
	{
		...
		logger.info( "[BPD] max age: " + maxAge + " days" );
		...
		try
		{
			...
			if ( days == 0 )
			{
				logger.info( "[BPD] auto-expiry disabled" );
				return false;
			}
		...
		if ( bpd != null )
		{
			...
			logger.info( "[BPD] last update: " + (lastUpdate == 0 ? "never" : new Date( lastUpdate )) );
		}

		long now = System.currentTimeMillis();
		if ( maxMillis < 0 || (now - lastUpdate) > maxMillis )
		{
			logger.info( "[BPD] expired, will be updated now" );
			return true;
		}
	...
	/**
	 * Aktualisiert die  Bank-Parameter-Daten (BPD) bei Bedarf.
	 */
	public void fetchBPD()
	{
		...
		if ( version.equals( "0" ) || isBPDExpired() || hbciVersionOfBPD == null || !hbciVersionOfBPD.equals( passport.getHBCIVersion() ) )
		{
			...
			try
			{
				...
				while ( true )
				{
					...
					if ( need_restart )
					{
						logger.info( "for some reason we have to restart this dialog" );
						if ( restarted )
						{
							logger.warn( "this dialog already has been restarted once - to avoid endless loops we stop here" );
				...
				if ( !status.isOK() )
				{
					logger.error( "fetching BPD failed: {}", status.getErrorString() );
			...
			catch ( Exception e )
			{
				...
				logger.info( "FAILED! - maybe this institute does not support anonymous logins" );
				logger.info( "we will nevertheless go on" );
			}
			finally
			{
				passport.closeComm();
		...
		logger.debug( "checking if requested hbci parameters are supported" );
		...
		{
			logger.warn( "can not check if requested parameters are supported" );
		}
	}
	...
	public void fetchKeys()
	{
		...
		if ( passport.needInstKeys() && !passport.hasInstEncKey() )
		{
			...
			try
			{
				...
				logger.info( "fetching institute keys" );
				...
				while ( true )
				{
					...
					boolean needRestart = passport.postInitResponseHook( status, true );
					if ( needRestart )
					{
						logger.info( "for some reason we have to restart this dialog" );
						if ( restarted )
				...
				if ( !status.isOK() )
				{
					logger.error( "fetching institute keys failed: {}", status.getErrorString() );
					throw new ProcessException( HBCIUtils.getLocMsg( "ERR_INST_GETKEYSFAILED" ), status );
				}
			}
			catch ( Exception e )
			{
				throw new HBCI_Exception( HBCIUtils.getLocMsg( "EXCMSG_FETCH_IKEYS_ERR" ), e );
			}
			finally
			{
				passport.closeComm();
