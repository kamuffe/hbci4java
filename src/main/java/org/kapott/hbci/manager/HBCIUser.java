		...
		implements IHandlerData
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCIUser.class );
	...
	private void sendAndActivateNewUserKeys( HBCIKey[] sigKey, HBCIKey[] encKey )
	{
		try
		{
			logger.info( "sending user keys to institute" );
			...
			if ( !passport.hasMySigKey() )
			{
				...
				if ( !ret.isOK() )
				{
					...
					{
						logger.warn( "deleting locally generated user keys");
					...
					{
						logger.warn( "keys have not been thrown away");
					}
	...
	public void manuallySetNewKeys( KeyPair sigKey, KeyPair encKey )
	{
		if ( passport.needUserKeys() )
		{
			...
			try
			{
				logger.info( "manually setting new user keys" );
	...
	public void fetchSysId()
	{
		try
		{
			...
			logger.info( "fetching new sys-id from institute" );
			...
			logger.debug( "new sys-id is " + passport.getSysId() );
	...
	public void fetchSigId()
	{
		try
		{
			...
			logger.info( "syncing signature id" );
			...
			logger.debug( "signature id set to " + passport.getSigId() );
	...
	{
		logger.debug( "extracting UPD from results" );
		...
		if ( p.size() != 0 )
		{
			...
			if ( upd != null )
			{
				...
				if ( mediaInfo != null )
				{
					logger.info( "rescued TAN media info to new UPD: " + mediaInfo );
			...
			logger.info( "installed new UPD [old version: " + oldVersion + ", new version: " + passport.getUPDVersion() + "]" );
	...
	public void fetchUPD()
	{
		try
		{
			...
			logger.info( "fetching UPD (BPD-Version: " + passport.getBPDVersion() + ")" );
