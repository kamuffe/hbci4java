		...
		extends SmartCardService
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( ChipTanCardService.class );
	 ...
	 * @throws Exception
	 */
	public String getTanScardControl( String hhduc )
			throws Exception
	{
		String tan = null;
...

		{
			...
			logger.info( "sending HHDuc to chipcard using ScardControl, length: {}", hhdBytes.length );
			...
			cmd.write( new byte[] { 0x00, (byte) hhdBytes.length } ); // HHD Laenge - kann eh nie laenger als 255 Zeichen sein - von daher
																		// reicht ein Byte aus
		...
		{
			logger.info( "finalizing chipcard transaction" );
			ByteArrayOutputStream cmd = new ByteArrayOutputStream();
	 ...
	 * @throws Exception
	 */
	public String getTanScardTransmit( String hhduc )
			throws Exception
	{
		String tan = null;
...

		{
			...
			logger.info( "sending HHDuc to chipcard using ScardTransmit, length: {}", hhdBytes.length );
	...
	public String parseTAN( byte[] bytes )
	{
		...
		{
			byte b1 = (byte) ((bytes[i] & 0xf0) >> 4); // mit 0xf0 UNDen, um das linke Halbbyte zu erhalten und dann 4 Bit nach rechts
														// verschieben
