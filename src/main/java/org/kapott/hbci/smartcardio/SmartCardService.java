...
public abstract class SmartCardService
{
	...
	protected enum Feature
	{
		 ...
		 * 
		 * @param b
		 *        das Byte mit der Feature-Nummer.
	 ...
	 * 
	 * @param type
	 *        der Typ des zu erzeugenden Service.
	 * @param name
	 *        optionale Angabe des Kartenlesernamens - noetig, wenn am Rechner mehrere Kartenleser angeschlossen sind.
	...
	public static <T extends SmartCardService> T createInstance( Class<? extends SmartCardService> type, String name )
	{
		...
		try
		{
			logger.info( "creating smartcard-service, using type " + type.getSimpleName() );
			...
			logger.info( "found card terminals:" );
			...
			if ( name != null )
			{
				logger.debug( "explicit terminal name given, trying to open terminal: " + name );
			...
			{
				logger.debug( "open first available card terminal" );
				terminal = list.get( 0 );
			}
			logger.debug( "using card terminal {}", terminal.getName() );
			...
			if ( Modifier.isAbstract( type.getModifiers() ) )
			{
				...
				logger.debug( " trying to load: {}", serviceName );
				type = (Class<SmartCardService>) Class.forName( serviceName );
			}
			else
			{
				logger.debug( " trying to load: {}", type.getName() );
			}

			SmartCardService cardService = type.newInstance();
			logger.info( " using: {}", cardService.getClass().getName() );
	...
	protected void init( Card card )
	{
		...
		try
		{
			...
			for ( int i = 0; i < response.length; i += 6 )
			{
				...
				if ( feature == null )
				{
					logger.info( "  unknown feature: " + Integer.toHexString( ioctl.intValue() ) );
					continue;
				}

				logger.info( "  " + feature.name() + ": " + Integer.toHexString( ioctl.intValue() ) );
				features.put( feature, ioctl );
			}
		}
		catch ( Exception e )
		{
			logger.warn( "unable to query features, continuing without having a feature set, error: {}", e.getMessage() );
			logger.warn( "", e );
		}
	}
	...
	protected byte[] receive( CommandAPDU command, byte[] returncodes )
	{
		try
		{
			...
			logger.debug( caller + " command : " + toHex( command.getBytes() ) );
			logger.debug( caller + " response: " + toHex( response.getBytes() ) );
