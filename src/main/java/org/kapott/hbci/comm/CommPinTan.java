		...
		extends Comm
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( CommPinTan.class );
	...
	public CommPinTan( HBCIPassportInternal parentPassport )
	{
		...
		try
		{
			...
			logger.info( HBCIUtils.getLocMsg( "LOG_CONNECT", new Object[] { host, parentPassport.getPort(), path } ) );
			...
			String proxyHost = ((AbstractPinTanPassport) parentPassport).getProxy();
			if ( proxyHost != null )
			{
				...
				if ( proxyData.length == 2 )
				{
					...
					logger.debug( "initializing HBCI4Java proxy authentication callback" );
	...
	protected void ping( MSG msg )
	{
		try
		{
			...
			if ( !checkCert || debugging )
			{
				...
				logger.debug( "activating modified socket factory for"
				...
				if ( !checkCert )
				{
					...
					logger.debug( "activating modified hostname verifier because cert checking is disabled" );
			...
			logger.debug( "writing data to output stream" );
			out.write( b );
			out.flush();
	...
	protected StringBuffer pong( MsgGen gen )
	{
		try
		{
			...
			logger.info( HBCIUtils.getLocMsg( "STATUS_MSG_RECV" ) );
			...
			if ( msgsize != -1 )
			{
				logger.debug( "found messagesize: " + msgsize );
			}
			else
			{
				logger.debug( "can not determine message size, trying to detect automatically" );
			}
			...
			while ( msgsize != 0 && (num = i.read( b )) > 0 )
			{
				...
				{
					logger.debug( "read " + num + " bytes, looking for more" );
				}
			}
