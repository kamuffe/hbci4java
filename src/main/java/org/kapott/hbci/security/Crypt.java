...
public final class Crypt
{
	...
	public MSG cryptIt( String newName )
	{
		...
		if ( passport.hasInstEncKey() )
		{
			...
			{
				logger.debug( "did not encrypt - message does not want to be encrypted" );				
			}
		}
		else
		{
			logger.warn( "can not encrypt - no encryption key available" );			
		}
	...
	{
		StringBuilder ret = new StringBuilder( msg.toString( 0 ) );
		HBCIPassportInternal passport = (HBCIPassportInternal) handlerdata.getPassport();
		...
		if ( passport.hasMyEncKey() )
		{
			if ( isCrypted() )
			{
				try
				{
					 ...
					 * throw new HBCI_Exception(HBCIUtils.getLocMsg("EXCMSG_CRYPTMETHODFAIL",new Object[]
					 * {keytype,passport.getSecMethod56()}));
					...
					logger.debug( "decrypted message: " + ret );
				}
				catch ( Exception ex )
				{
			...
			{
				logger.debug( "did not decrypt - message is already cleartext" );				
			}
		}
		else
		{
			logger.warn( "can not decrypt - no decryption key available" );			
		}
