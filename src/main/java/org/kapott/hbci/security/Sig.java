...
public final class Sig
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( Sig.class );
	...
	public boolean signIt()
	{
		...
		if ( mainPassport.hasMySigKey() )
		{
			...
			{
				logger.debug( "did not sign - message does not want to be signed" );				
			}
		...
		{
			logger.warn( "can not sign - no signature key available" );			
		}
	...
	private void readSigHead()
	{
		...
		{
			logger.warn( "missing bank code in message signature, ignoring..." );
		}
	...
	public boolean verify()
	{
		...
		if ( mainPassport.hasInstSigKey() )
		{
			...
			if ( dontsignAttr.length() == 0 )
			{
				...
				{
					logger.warn( "message has no signature" );
					/*
			...
			{
				logger.debug( "message does not need a signature" );
				ret = true;
			}
		}
		else
		{
			logger.warn( "can not check signature - no signature key available" );
			ret = true;
		}
