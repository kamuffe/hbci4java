		...
		extends HBCIJobImpl
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( GVTAN2Step.class );
	...
	protected void saveReturnValues( HBCIMsgStatus status, int sref )
	{
		...
		if ( origTask != null )
		{
			...
			logger.debug( "storing return values in orig task (segnum=" + orig_segnum + ")" );
	...
	protected void extractResults( HBCIMsgStatus msgstatus, String header, int idx )
	{
		...
		logger.debug( "found HKTAN response with segcode " + segcode );
		...
		if ( origTask != null && new StringBuffer( origTask.getHBCICode() ).replace( 1, 2, "I" ).toString().equals( segcode ) )
		{
			...
			logger.debug( "this is a response segment for the original task - storing results in the original job" );
			origTask.extractResults( msgstatus, header, idx );
		}
		else
		{
			logger.debug( "this is a \"real\" HKTAN response - analyzing HITAN data" );

			String challenge = result.getProperty( header + ".challenge" );
			if ( challenge != null )
			{
				logger.debug( "found challenge '" + challenge + "' in HITAN - saving it temporarily in passport" );
			...
			if ( hhdUc != null )
			{
				logger.debug( "found Challenge HHDuc '" + hhdUc + "' in HITAN - saving it temporarily in passport" );
			...
			if ( orderref != null )
			{
				...
				logger.debug( "found orderref '" + orderref + "' in HITAN" );
				if ( otherTAN2StepTask != null )
				{
					...
					logger.debug( "storing it in following HKTAN task" );
					otherTAN2StepTask.setParam( "orderref", orderref );
				}
				else
				{
					logger.debug( "no other HKTAN task known - ignoring orderref" );
				}
			}
		}
	}
