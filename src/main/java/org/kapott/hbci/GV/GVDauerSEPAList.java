		...
		extends AbstractSEPAGV
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( GVDauerSEPAList.class );
	...
	protected void extractResults( HBCIMsgStatus msgstatus, String header, int idx )
	{
		...
		logger.info( "parsing SEPA standing orders from msg data [size: " + result.size() + "]" );
		...
		try
		{
			...
			logger.debug( "  parsing sepa data: " + pain );
			parser.parse( new ByteArrayInputStream( pain.getBytes( Comm.ENCODING ) ), sepaResults );
			logger.info( "  parsed sepa data, entries: " + sepaResults.size() );
		}
		catch ( Exception e )
		{
			logger.error( "  unable to parse sepa data: " + e.getMessage() );
			throw new HBCI_Exception( "Error parsing SEPA pain document", e );
		}

		if ( sepaResults.isEmpty() )
		{
			logger.warn( "  found no sepa data" );
			return;
		}
