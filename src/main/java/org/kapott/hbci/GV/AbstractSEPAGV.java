		...
		extends HBCIJobImpl
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( AbstractSEPAGV.class );
	 ...
	 * @param handler
	 * @param gvName
	 *        der Geschaeftsvorfall fuer den in den BPD nach dem PAIN-Versionen
	 *        gesucht werden soll.
	...
	private SepaVersion determinePainVersion( HBCIHandler handler, String gvName )
	{
		...
		if ( globalVersion == null && jobVersion == null )
		{
			...
			logger.warn( "unable to determine matching pain version, using default: " + def );
			return def;
		}
		...
		if ( jobVersion == null )
		{
			logger.debug( "have no job-specific pain version, using global pain version: " + globalVersion );
			return globalVersion;
		}

		// Ansonsten hat die vom Job Vorrang:
		logger.debug( "using job-specific pain version: " + jobVersion );
		return jobVersion;
	}
	...
	{
		logger.debug( "searching for supported pain versions for GV " + gvName );

		if ( handler.getSupportedLowlevelJobs().getProperty( gvName ) == null )
		{
			logger.debug( "don't have any BPD for GV " + gvName );
			return null;
		}
		...
		while ( e.hasMoreElements() )
		{
			...
			{
				logger.warn( "ignoring invalid pain version " + urn );
	...
	protected void setLowlevelParam( String key, String value )
	{
		...
		if ( key.startsWith( intern ) )
		{
			...
			logger.debug( "setting SEPA param " + realKey + " = " + value );
		}
		else
		{
	...
	protected void createSEPAFromParams()
	{
		...
		try
		{
			...
			logger.debug( "schema validation enabled: " + validate );
