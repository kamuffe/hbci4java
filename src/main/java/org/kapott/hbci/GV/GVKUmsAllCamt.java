		...
		extends AbstractSEPAGV
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( GVKUmsAllCamt.class );
	...
	private String getStartdate()
	{
		...
		logger.info( "earliest start date according to BPD: " + (date != null && date.length() > 0 ? date : "<none>") );
		return date;
	}
	...
	protected void extractResults( HBCIMsgStatus msgstatus, String header, int idx )
	{
		...
		for ( int i = 0;; i++ )
		{
			...
			try
			{
				...
				logger.debug( "  parsing camt data: " + booked );
				result.camtBooked.add( booked );
				parser.parse( new ByteArrayInputStream( booked.getBytes( Comm.ENCODING ) ), result.getDataPerDay() );
				logger.debug( "  parsed camt data, entries: " + result.getFlatData().size() );
			}
			catch ( Exception e )
			{
				logger.error( "  unable to parse camt data: " + e.getMessage() );
		...
		if ( notbooked != null )
		{
			try
			{
				...
				logger.debug( "  parsing unbooked camt data: " + notbooked );
				...
				logger.debug( "  parsed unbooked camt data, entries: " + result.getFlatDataUnbooked().size() );
			}
			catch ( Exception e )
			{
				logger.error( "  unable to parse unbooked camt data: " + e.getMessage() );
				throw new HBCI_Exception( "Error parsing CAMT document", e );
			}
		}
	}
