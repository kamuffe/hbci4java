 ...
 * </p>
 * <p>
 * In der Regel wird zunächst ein <code>DTAUS</code>-Objekt erzeugt. Dazu
...
public class DTAUS
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( DTAUS.class );
	 ...
	 *        </ul>
	 * @param execdate
	 *        Ausführungsdatum für diesen Sammelauftrag; <code>null</code>,
	...
	{
		logger.debug( "parsing DTAUS data" );
		...
		while ( true )
		{
			...
			posi += 4;
			logger.debug( "SetCLen = " + setCLen + " data bytes (--> " + ((setCLen - 187) / 29.0) + " extensions)" );
			...
			posi += 2;
			logger.debug( "field 'nofExtensions' = " + nofExtensions );
		...
		logger.debug( "parsinng of DTAUS data finished" );
	}
}
