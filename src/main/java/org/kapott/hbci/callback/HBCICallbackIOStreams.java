		...
		extends AbstractHBCICallback
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCICallbackIOStreams.class );
...
//					break;
				...
				case HAVE_HARDPIN:
					logger.debug( "end of entering hardpin" );
					break;

				case HAVE_CHIPCARD:
					logger.debug( "end of waiting for chipcard" );
					break;
...
//					break;
				...
				case NEED_NEW_INST_KEYS_ACK:
					...
					getOutStream().println( HBCIUtils.getLocMsg( "HASH" ) + ": " + HBCIUtils.data2hex( iniletter.getKeyHashDisplay() ) );
	...
	public synchronized void status( HBCIPassport passport, int statusTag, Object[] o )
	{
		switch ( statusTag )
		{
			...
			case STATUS_MSG_RAW_RECV_ENCRYPTED:
				getOutStream().println( HBCIUtils.getLocMsg( "STATUS_MSG_RAW_RECV_ENCRYPTED", o[0].toString() ) );
				break;
