 ...
 * </p>
 * <p>
 * Mehr Informationen sind in der Datei <code>README.ThreadedCallbacks</code>
		...
		extends AbstractHBCICallback
{
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger( HBCICallbackThreaded.class );
	 ...
	 * 
	 * @param realCallback
	 *        eine Instanz einer "normalen" Callback-Klasse. Alle asynchron
	...
	{
		logger.debug( "hbci thread: threaded callback received" );
		...
		if ( sync_main == null )
		{
			...
			logger.debug( "hbci-thread: non-execute-callback, calling real callback" );
			realCallback.callback( passport, reason, msg, datatype, retData );
		}
		else
		{
			logger.debug( "hbci-thread: this callback is triggered in HBCIHandler.execute()" );
			...
			if ( realCallback.useThreadedCallback( passport, reason, msg, datatype, retData ) )
			{
				...
				logger.debug( "hbci thread: callback: awaking main thread with callbackData" );
				...
				logger.debug( "hbci thread: callback: waiting for callback response from main thread;" );
				...
				logger.debug( "hbci thread: callback: got response from main thread - returning callback answer to kernel" );
